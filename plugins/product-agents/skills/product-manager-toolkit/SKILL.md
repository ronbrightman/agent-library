...
**Local customization (2026-07-20): Effort is split into two dimensions,
not one.** Classic RICE has a single `Effort` term. This copy splits it:

- `engineering_effort` — coding/implementation work, done by Claude Code,
  not the founder personally. Weighted **lightly** (0.5x — half of
  classic RICE's Effort weight) since it doesn't cost the founder's own
  time.
- `personal_effort` — decisions, approvals, signups, anything that needs
  the founder's actual attention. Weighted at **full/normal** strength
  (1.0x, same as classic RICE's Effort term), since this is the founder's
  genuinely scarce resource.

Each feature's score reports a `binding_constraint`: `build_time`,
`personal_time`, or `balanced` — which dimension is actually holding the
score down. Use this to distinguish "deprioritized because it's a big
build" from "deprioritized because it eats my time" — those are different
problems and should be surfaced differently in a recommendation. See the
docstring at the top of `scripts/rice_prioritizer.py` for the exact
formula and weights.

**Features:**
- RICE score calculation with independently-weighted engineering vs.
  personal effort
- Portfolio balance analysis (quick wins vs big bets vs. "high-impact but
  personal-time-blocked")
- Quarterly roadmap generation against two separate capacities
  (engineering person-months AND personal hours — a quarter can fill up
  on either one)
...
User Dashboard Redesign,5000,high,high,l,s,Complete redesign
Mobile Push Notifications,10000,massive,medium,m,xs,Add push support
Dark Mode,8000,medium,high,s,xs,Dark theme option
Payment Processor Integration,6000,high,medium,l,xl,New vendor - needs underwriting approval
```
...
# Run with default capacities (10 engineering person-months, 20 personal hours/quarter)
python scripts/rice_prioritizer.py features.csv

# Custom capacities
python scripts/rice_prioritizer.py features.csv --capacity 20 --personal-capacity 15

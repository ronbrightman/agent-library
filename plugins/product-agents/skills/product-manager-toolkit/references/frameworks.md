...
**Formula (local customization, 2026-07-20 — effort split in two):**
```
RICE Score = (Reach × Impact × Confidence) / (EngineeringEffort^0.5 × PersonalEffort^1.0)
```

The classic single `Effort` term is split into two independently-scored,
independently-weighted dimensions — see `scripts/rice_prioritizer.py`'s
docstring for the full rationale and implementation:

| Component | Description | Values | Weight |
|-----------|-------------|--------|--------|
...
| **Engineering effort** | Person-months of build work (Claude Code's time, not the founder's) | xl=13, l=8, m=5, s=3, xs=1 | **0.5x** (half weight) |
| **Personal effort** | Hours of founder decisions/approvals/signups/attention | xl=13, l=8, m=5, s=3, xs=1 | **1.0x** (full weight) |

Engineering effort is dampened because it's delegated and doesn't cost
the founder personally; personal effort stays at full classic-RICE
weight because it's the founder's genuinely scarce resource. Every score
also reports a `binding_constraint` (`build_time` / `personal_time` /
`balanced`) so a low-ranked idea can be correctly attributed to "this
needs too much of my time" vs. "this is a big build" — those call for
different responses.
...
Engineering effort: medium (5 person-months) -> 5^0.5 = 2.236
Personal effort: extra-small (1 hour) -> 1^1.0 = 1

RICE = (10,000 × 3 × 0.8) / (2.236 × 1) = 10,733
```
Compare to the same feature with a heavier personal-effort requirement
(e.g. a new vendor integration needing underwriting approval, `l` = 8
hours): the denominator becomes `2.236 × 8 = 17.89`, dropping the score
to ~1,341 — over 8x lower, driven entirely by the founder-time term, with
engineering effort unchanged. `binding_constraint` would report
`personal_time` for that version and `build_time` for the original.
...
Because these thresholds were calibrated against classic single-Effort
RICE, scores under the split formula run somewhat higher on average
(engineering effort's dampened weight shrinks the denominator versus
before) — treat the bands above as directional, not exact, until you've
run enough real features through the split formula to recalibrate them.
